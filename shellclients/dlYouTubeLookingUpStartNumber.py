...
Explanation: 
  ...
  1) it sets --playlist-start to the number of local folder mp4 files minus 1
     (minus 1 because the last one may be incomplete, though the id's change due to
      new submitted videos and some videos may end up incomplete)
...
def continue_youtubedl_until_retval_is_zero():
  ...
  while ret_val != 0:
    loop_seq += 1
    start_n = get_start_n()
    line_msg = '%s Downloading with start_n = %d' % (time.ctime(), start_n)
    print_and_log(line_msg)
    comm = BASE_COMMAND % {'start_n': start_n}
    line_msg = 'seq = %d :: %s' % (loop_seq, comm)
    print_and_log(line_msg)
