...
class VideoidsGrabberAndDownloader:
  """
  ...
  def __init__(self, youtubeids_filename=None, to_folder='.'):
    self.to_folder = to_folder
    self.current_folder = None
    self.chdir()
    self.youtubeids_filename = None
  ...
  def chdir(self):
    if self.to_folder == '.':
      ...
      return
    if os.path.isdir(self.to_folder):
      self.current_folder = self.to_folder
      os.chdir(self.to_folder)

  def set_youtubeids_filename(self, youtubeids_filename=None):
    if youtubeids_filename is None or not os.path.isfile(youtubeids_filename):
  ...
  def find_alternative(self):
    ...
    for eachfile in files:
      match_obj = idstr_re.search(eachfile)
      if match_obj:
        vid = match_obj.group(1)
        if vid not in vids:
          print('Found vid', vid)
          vids.append(vid)
    self.all_videoids_here = vids
        ...
        if os.path.isdir(current_abspath):
          ...
          for dir_content in dir_contents:
            if dir_content.startswith(self.PREFIX_TO_LS_R_CONTENTS_TEXT_FILE):
              self.add_videoids_from_contents_text_file(dir_content, self.current_folder)
        else:
  ...
  def issue_download(self, n_seq, vid):
    ...
    while n_tries <= self.MAX_N_TRIES:
      print(n_seq, 'n.of dl. so far', self.n_downloaded, 'of', len(self.videoids_to_download),
            'video', vid, 'n_tries', n_tries, '[%s]' % time.ctime())
