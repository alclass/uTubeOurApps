#!/usr/bin/env python3
"""
  In March 2021, this script was refactored from Python 2 to Python 3
  # -*- coding: utf-8 -*-
...
import sys
import unittest
from sabdir.SabDirLectureFilenameInfoerMod import SabDirLectureFilenameInfoer
...
class DownDirTreeSabDirMp3splitFolderFinder(object):
  ...
  def walk_down_dirtree(self):
    ...
    for dirpath, dirnames, _ in os.walk(self.basedir_abspath):
      ...
      if mp3folderfound is not None:
        ...
        # usually when, in var.split(), var is None,
        # having no attribute (it will probably never happen here, for dirpath comes from os.walk())
  ...
  def remove_all_collected_folders(self):
    ...
    ans = input('Are you sure you want to erase all %d folders above? (y/N*) ' % len(self.mp3split_abspaths))
    if ans.lower() != 'y':
...
def get_target_basedir_abspath_from_sysargv_or_default():
  ...
  if target_base_abspath is None:
...
def process():
  ...
  _ = get_target_basedir_abspath_from_sysargv_or_default()
