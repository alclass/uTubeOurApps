...
class Renamer:
  ...
  def __init__(self, youtube_filenames_as_title_plus_videoid_file_abspath=None):
    self.youtube_filenames_as_title_plus_videoid_file_abspath = None
  ...
  def init_youtube_filenames_as_title_plus_videoid_file_abspath(
      self,
      youtube_filenames_as_title_plus_videoid_file_abspath
    ):
    self.youtube_filenames_as_title_plus_videoid_file_abspath = self.DEFAULT_TXT_FILE_WITH_NEW_FILENAMES
    if youtube_filenames_as_title_plus_videoid_file_abspath is not None:
    ...
    if not os.path.isfile(self.youtube_filenames_as_title_plus_videoid_file_abspath):
      error_msg = 'YoutubeRenameDataFileDoesNotExist (file: [%s])' % \
                  self.youtube_filenames_as_title_plus_videoid_file_abspath
      raise YoutubeRenameDataFileDoesNotExist(error_msg)
  ...
  def please_confirm_batch_rename(self):
    ...
    ans = input(' Y*/n (Obs: only "n" or "N" will stop renaming.) ')
    if ans in ['n', 'N']:
      sys.exit(0)
