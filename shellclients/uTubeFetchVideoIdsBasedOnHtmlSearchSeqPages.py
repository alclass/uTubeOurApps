...
# -*- coding: utf-8 -*-
"""
This script forms file youtube-ids.txt which is input for a second script that reuses youtube-dl and download the
youtube videos by their id's, one at a time
"""
import glob
import os  # sys
...
# filename_with_increment_interpolation, n_total_pages = input_data.filename_with_increment_interpolation,
# input_data.n_total_pages
...
def process():
  ...
  + There are no youtube_search_files on this folder
    (they should end with " - YouTube <n>.htm[l]" where <n> is a number).
  + Please look into the directory listing to verify there is at least one of them. 
    (In case there are any of them, please report a bug in this program.
'''
    print(msg)
    return
