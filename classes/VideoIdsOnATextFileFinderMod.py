...
def filter_filenames_under_dot_extension(files, dot_extension):
  filtered_file_list = filter(lambda f: f.endswith(dot_extension), files)
  return filtered_file_list


class VideoIdsOnFolderFilenamesFinder:
  """
  ...
  def set_videoids_folder_abspath(self, videoids_folder_abspath=None):
    if videoids_folder_abspath is None or not os.path.isdir(videoids_folder_abspath):
      self.videoids_folder_abspath = os.path.abspath('.')
      return
  ...
  def extract_videoids_from_filenames_on_folder(self, reread=False):
    ...
    """
    if self.videoids_and_extractors_for_folderfilenames_dict is not None and not reread:
      return 
  ...
  def get_folder_filenames_with_a_videoid_by_dot_extension(self, dot_extension=None):
    ...
    if None in filenames:
      error_msg = 'This is apparently a program or logic error. ' \
                  'There is a None inside a list coming from get_all_folder_filenames_with_a_videoid()'
      raise ValueError(error_msg)
    return filter_filenames_under_dot_extension(filenames, dot_extension)
...
class VideoIdsOnATextFileFinder(VideoIdsOnFolderFilenamesFinder):
  ...
  def set_textfile_abspath(self, textfile_abspath=None):
    if textfile_abspath is None or not os.path.isfile(textfile_abspath):
  ...
  def read_and_set_all_filenames_from_videoids_textfile(self, reread=False):
    if self.all_filenames_inside_textfile is not None and not reread:
      return self.all_filenames_inside_textfile
    # f = codecs.open(self.videoids_textfile_abspath, 'r', encoding='utf-8')
    f = open(self.videoids_textfile_abspath, 'r', encoding='utf-8')
    filetext = f.read()
    f.close()
  ...
  def extract_videoids_from_textfile(self, reread=False):
    if self.videoids_and_extractors_for_textfile_dict is not None and not reread:
      return
  ...
  def find_videoids_on_textfile_missing_on_folder_files(self, reread=False):
    """
    """
    if self.videoids_on_textfile_missing_on_folder_files is not None and not reread:
      return
    self.extract_videoids_from_textfile(reread)
    self.extract_videoids_from_filenames_on_folder(reread)
  ...
  def get_videoids_and_extractors_from_textfile(self, reread=False):
    if self.videoids_and_extractors_for_textfile_dict is None or reread:
      self.extract_videoids_from_textfile(reread)
    return self.videoids_and_extractors_for_textfile_dict
  ...
  def download_missing_videos(self):
    """
    """
    if self.videoids_on_textfile_missing_on_folder_files is None:
...
def read_test_videoids_file_create_it_if_needed(n_of_retries=0):
  ...
  if os.path.isfile(videoids_file_abspath):
    f = open(videoids_file_abspath, 'r', encoding='utf-8')
  ...
  f = open(videoids_file_abspath, 'w', encoding='utf-8')
...
class TestFilenameVideoidExtractor(unittest.TestCase):
  ...
  def test_3_raise_ValueError_for_None_filepath(self):
    ...
    _ = videoids_inside_a_file_finder.get_total_videoids_from_textfile()
...
def process():
  ...
  print('get_videoids_on_textfile_missing_on_folder_files() =',
        videoids_inside_a_file_finder.get_videoids_on_textfile_missing_on_folder_files())
